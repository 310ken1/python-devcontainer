...
```bash
api-mock$ sam build --use-container
```
...
```bash
api-mock$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
api-mock$ sam local start-api
api-mock$ curl http://localhost:3000/
```
...
```bash
api-mock$ sam logs -n HelloWorldFunction --stack-name api-mock --tail
```
...
```bash
api-mock$ pip install -r tests/requirements.txt --user
# unit test
api-mock$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
api-mock$ AWS_SAM_STACK_NAME="api-mock" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "api-mock"
```
